...
> 我在 React 项目中通常会将请求逻辑封装成自定义 Hook，比如 useRequest，通过参数控制请求方式、是否自动请求、loading 状态、错误处理等，支持 GET/POST 和缓存逻辑，并结合 Axios 和 React Query 等库使用。在项目中可以大大减少重复代码，提升开发效率。
...
export function useRequest<T = any>(
  ...
  const run = useCallback(
    async (overrideConfig?: AxiosRequestConfig) => {
      setLoading(true);
      try {
        const res = await axios({ ...config, ...overrideConfig });
        setData(res.data);
        setError(null);
      } catch (err) {
        setError(err);
...
// 需要时手动触发请求
const handleClick = () => run({ params: { id: 1 } });
```

## React 的渲染机制与性能优化

1. 当组件的 state 或 props 发生变化时，React 是如何进行渲染的？

   - 首先通过 VDOM 生成新的虚拟 DOM 树
   - 然后通过 Diff 算法对比新旧虚拟 DOM 树
   - 在痛殴 Reaconiliation(协调)算法找出变化的部分
   - 最后在 commit 阶段执行实际的 DOM 更新
     **批量的、异步的、最小化更新**

2. 你在实际项目中遇到过性能瓶颈吗？你是怎么定位并优化的？
   在表格封装时，我发现切换分页时页面明显卡顿，使用 Chrome DevTools 的 Performance 面板分析后，发现：
   - 多个子组件在 props 没变化的情况下仍被重新渲染；
   - 部分函数定义在组件体内，导致每次渲染都生成新引用。
     为此我做了如下优化：
     - 使用 React.memo 包裹纯组件，阻止不必要渲染；
     - 对传递给子组件的函数使用 useCallback 缓存；
     - 对计算开销大的值使用 useMemo 缓存；
     - 使用 Profiler + DevTools 定位 render 次数与耗时变化。

最终，渲染次数减少 60%，表格切换平均耗时从 280ms 降到 110ms，页面更流畅。

1. 请讲讲 key 的作用以及你踩过的坑？
   **key 的作用: 准确 diff、减少重排、避免状态错乱**
   React 在 diff 阶段依赖 key 来准确识别每个列表项：
   - 正确设置 key（如唯一 ID）可以避免组件被误复用或重建；
   - 如果使用 index 作为 key，可能会导致：
   - 组件状态错乱（如输入框丢失焦点）；
   - 无法正确复用导致性能下降。

我曾踩过用 index 当 key 的坑，列表顺序变化后组件状态错乱，后续统一改为使用唯一业务 ID 解决问题。

## 你了解 useState 的底层实现原理吗？为什么它必须写在函数组件的最顶层？

React 通过内部维护一个“Hooks 调用栈”（Hook list），每次渲染时根据调用顺序来定位对应的 state 或 effect。

所以：

- `useState` 是按调用顺序记录的（不是按名字）；
- 这就是为什么我们**不能在条件语句、循环里使用 useState/useEffect** —— 会破坏调用顺序，导致状态错乱。

底层上，每次组件 render，React 会调用函数组件，把每个 Hook 绑定到内部的 fiber 节点上，按顺序保存状态。

## useEffect 和 useLayoutEffect 有什么区别？它们分别在什么时候执行？

| 特性     | useEffect               | useLayoutEffect                                 |
| -------- | ----------------------- | ----------------------------------------------- |
| 执行时机 | 异步,在浏览器绘制后执行 | 同步，在 DOM 更新后、浏览器绘制前执行           |
| 是否阻塞 | 不会阻塞 UI 渲染        | 会阻塞页面绘制                                  |
| 适合场景 | 请求、事件监听、订阅等  | DOM 测量/动画前处理，如获取元素宽高、滚动处理等 |

## 在项目中你是否遇到过 useEffect 执行顺序或依赖坑？怎么解决？

```ts
/**
 *  ❌1. 忘记加依赖项，导致副作用执行不及时或逻辑异常
 *  ✅ 解决：使用 ESLint 插件 eslint-plugin-react-hooks 强制依赖校验。
 */
useEffect(() => {
  fetchData(params); // params 未加到依赖中，数据不刷新
}, []); // ❌

/**
 *  ❌ 2. 依赖项引用频繁变动，导致频繁触发 effect
 * ✅ 解决方案：
 * 使用 useMemo 缓存依赖对象；
 * 或将对象扁平化为基本类型依赖项。
 */
useEffect(() => {
  doSomething();
}, [filters]); // filters 是对象，每次 render 地址都变
...
## Redux 和 React 的 Context API

### 区别

#### Redux 常用于全局状态管理(复杂可预测可调试)

- 核心机制: store、reducer、dispatch、middleware
- 利用中间件精准更新
- 支持 Redux DevTools,全量立即追踪
- 支持异步、日志、权限等中间件机制

#### Context API(跨层级组件传值（轻量、简洁）)

- Provider / Consumer，基于 React context
- context 更新会导致所有消费者组件重新渲染
- 不具备中间件能力
...
- 涉及多个模块的全局共享数据（如登录用户权限、系统配置）；
- 页面存在大量异步状态交互（表格筛选、接口联动）；
- 状态流需要进行权限校验、节流、日志埋点等操作。

因此我选用了 Redux Toolkit 搭配 redux-saga 实现异步处理，状态集中管理，且配合 Redux DevTools 调试体验非常好。

而在某些小模块（如主题切换、语言国际化），我使用 React Context 实现状态共享，减少冗余引入。

### 你有没有自己封装过 Redux 模块或中间件？可否举例说明？

#### Redux Module 封装

- 使用 createSlice 统一封装模块（如 userSlice, tableSlice）；
- 所有模块统一使用 namespace 命名，结合 combineReducers 自动注册；
- 封装 useAppSelector / useAppDispatch 提供类型推导。

#### Redux 中间件实践

- 自定义权限校验中间件：拦截特定 action 类型（如 route/jump），判断是否有权限；
- 自定义日志上报中间件：每次重要状态变更后，将变更日志埋点发送至日志系统；
- 自定义节流中间件：限制某些高频 dispatch（如 resize、input）执行频率。

这些封装显著提高了状态管理的安全性、可维护性和扩展性。

## 🔧 Redux 中间件原理与手写实现

### 1. Redux 中间件原理简述

Redux 的中间件本质上是一个高阶函数链式调用机制，结构如下：
...
- 使用 redux-thunk 处理接口异步；
- 使用 redux-saga 管理复杂副作用（如并发请求、取消、状态监听）；
- 自定义中间件：
  - 权限拦截中间件
  - 埋点上报中间件
  - 防抖中间件（处理输入、滚动）
...
- Redux 中间件结构：`store => next => action => result`
- 用于扩展 dispatch 行为，不改变原 reducer
- 常见：日志、异步、权限、节流、埋点
